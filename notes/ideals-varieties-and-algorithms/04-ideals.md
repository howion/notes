...
A subset $S \subseteq \mathbb{F}[\bar{x}]$ is an **ideal** of $\mathbb{F}[\bar{x}]$ if
...
$$
\Braket{f_1, ..., f_s} := \Set{\sum_{i=1}^{s}h_i f_i \mid h_1, ..., h_s \in \mathbb{F}[\bar{x}]}.
$$
...
$$
\bold{I}(V) := \Set{f \in \mathbb{F}[x_1, ..., x_n] \mid f(a_1, ..., a_n) = 0 \quad \forall (a_1, ..., a_n) \in V}
$$
